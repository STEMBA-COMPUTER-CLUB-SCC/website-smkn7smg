<section id="berita">
  <div
    ...
    <div class="w-max flex flex-col items-center mb-16 max-sm:mb-8">
      ...
      <p
        class="font-manrope font-normal text-[20px] text-[#525252] text-center w-[462px] max-sm:w-[265px] max-sm:text-[14px]">
    ...
    <div class="w-max flex flex-row gap-x-3">
      ...
      @endphp
      <div class='max-sm:w-screen max-sm:overflow-scroll scrollbar-hide'>
        <div class='flex flex-row items-center w-max gap-x-3 max-sm:gap-x-2 max-sm:pl-[8vw] max-sm:pr-[8vw]'>
          @foreach($categories as $category)
          <p wire:click="toggleCategory('{{ $category }}')"
            class="hover:cursor-pointer font-manrope font-semibold text-[16px] max-sm:text-[10px] max-sm:px-4 py-2 px-5 rounded-full border border-[#EBEBEB] w-max transition-all duration-300 {{ in_array($category, $selectedCategories) ? 'bg-[#FFFFFF] text-[' . ($colors[$category] ?? '#1152D9') . ']' : 'bg-[#F7F7F7] text-[#525252]' }}">
            {{ $category }}
          </p>
          @endforeach
        </div>
  ...
  <div
    ...
    <div wire:click="goToDetail({{ $item['id'] }})"
      ...
      <p
        class='font-manrope text-[20px] max-sm:text-[18px] max-sm:max-w-[295px] max-sm:mb-2 font-semibold max-w-[368px] m-0 break-word uppercase line-clamp-2 mb-3'>
        {{ $item["title"] }}</p>
      <p
        class='font-manrope text-[14px] max-sm:text-[16px] text-[#525252] max-sm:w-[295px] font-normal w-[368px] m-0 break-word line-clamp-2'>
        {{ $item["content"] }}</p>
    </div>
    @empty
    <p class="font-manrope text-[16px] text-[#525252] max-sm:text-[12px] col-span-3 text-center">Tidak ada berita yang
      ditemukan.</p>
    @endforelse
  </div>
  <div
